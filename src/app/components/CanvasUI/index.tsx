  ...
  useFrame((state) => {
    ...
    if (scene === "details") {
      controls.object.position.lerp(
        new Vector3(-clickedObj.x / 4, 0, -clickedObj.z / 4),
        0.017,
      );
...
export const Cactus_free = (props) => {
  ...
  const model = useLoader(
    OBJLoader,
    "/models/cactus_free/10436_Cactus_v1_max2010_it2.obj",
  );
...
export const CanvasUI = ({ scene, setScene }) => {
  ...
  return (
    <div
      className={`fixed w-full h-full ${scene === "cover" ? "" : "bg-[coral]/50"}`}
    >
      <Canvas>
